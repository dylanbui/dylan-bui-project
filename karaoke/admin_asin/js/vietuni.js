...
function setTypingMode(mode) {
  ...
  if (!supported && !disabled) {
    alert("Xin loi, trinh duyet cua ban khong cho phep dung VietTyping.\n");
    disabled = true; }}
...
function findCharToChange(type) {
  ...
  pc= lastchars[i][0];
  if ((i==0 || chr==0) && type!=15) chr= this.charmap.isVowel(pc);
  if (!chr) return null;
  var clen= lastchars[i][1], isuo=0;
  if (type==7 || type==8 || type==11) {
    c= (i>0)? lastchars[i-1][0] : 0;
    ppc= (i<=6)? lastchars[i+1][0] : 0;
    if (isuo=this.charmap.isUO(pc,c)) clen+=lastchars[i-1][1];
    else if (isuo=this.charmap.isUO(ppc,pc)) clen+=lastchars[++i][1];
    if (isuo) { chr=isuo; isuo=1; }}
...
CVietCharMap.prototype.getAEOWD = function (ind, type, isuo) {
  ...
  if (isuo) {
    ...
    if (type==7 || type==11) c= this.charAt(10)+this.charAt(base+7);
    else if (type==8) c= this.charAt(11)+this.charAt(base+8);
    return [c!=0, c, 1];}
